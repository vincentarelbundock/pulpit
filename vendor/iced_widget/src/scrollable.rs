...
impl Scrollbar {
    ...
    /// Sets the minimum length of the draggable scroller.
    ...
    fn new(
        ...
        let y_scrollbar = if let Some(vertical) = show_scrollbar_y {
            ...
            let scroller = if ratio >= 1.0 {
                ...
                let scroll_range = (content_bounds.height - bounds.height).max(0.0);
                let track_range = (scrollbar_bounds.height - scroller_height).max(0.0);
                let scroller_offset = if scroll_range > 0.0 {
                    translation.y / scroll_range * track_range
                } else {
                    0.0
                };
        ...
        let x_scrollbar = if let Some(horizontal) = show_scrollbar_x {
            ...
            let scroller = if ratio >= 1.0 {
                ...
                let scroll_range = (content_bounds.width - bounds.width).max(0.0);
                let track_range = (scrollbar_bounds.width - scroller_length).max(0.0);
                let scroller_offset = if scroll_range > 0.0 {
                    translation.x / scroll_range * track_range
                } else {
                    0.0
                };
