...
launch-release: ## Launch the optimised build — the only one worth timing (optionally: DECK=deck.pdf)
	nix develop . --command $(CARGO) run --release -p pulpit -- --no-restore $(DECK)
