...
/// The presenter searches through the render backend, and finds the same
/// thing the reader's document engine would: the fixture writes each page's
/// own number as its only run of text, so "3" is on page three and nowhere
...
fn the_render_backend_finds_text_in_a_real_deck() {
    ...
    let hits = backend
        .find_text(
            document,
            &pulpit_core::search::Query::new("3", false, false),
            0..5,
        )
        .expect("a real deck has a text layer");
    assert_eq!(hits.len(), 1, "expected one hit, got {hits:?}");
    assert_eq!(hits[0].page, pulpit_core::page::PageIndex(2));
    assert!(
        !hits[0].quads.is_empty(),
        "a hit with no quads marks nothing"
    );
    ...
    // Nothing to find is an empty answer, not an error: the query is fine,
    // the deck simply does not say it.
    assert!(backend
        .find_text(
            document,
            &pulpit_core::search::Query::new("zzzz", false, false),
            0..5,
        )
        .expect("no matches is not a failure")
        .is_empty());
}
