...
pub(crate) use overview::OverviewGrid;
pub(crate) use print::PrintJob;
use thumbnails::THUMBNAIL_BUDGET_BYTES;
pub(crate) use thumbnails::THUMBNAIL_WIDTH;
...
use pulpit_display::{Role, RoleTarget, WindowMode};
...
pub struct App {
    ...
    /// the dialog's own state machine, the identity it is signing with, the
    /// append-only gate and the signature panel. Grouped into one field
    /// (§79.4) so a reviewer does not have to hold a dozen flat fields
    /// together to know what "signing" means; see [`sign::SignState`].
    pub sign: sign::SignState,
    ...
    thumbnail_plan_inputs: Option<thumbnails::ThumbnailPlanInputs>,
...
impl App {
    ...
    fn is_live(&self) -> bool {
        ...
        self.uploads_settle_by.is_some_and(|at| self.now < at)
            ...
            // already finished.
            || self.sign.saving_since.is_some()
    ...
    fn dispatch(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::PasteFormText { focus, value } => self.handle_paste_form_text(focus, value),
            ...
            Message::AcceptAppendOnly => {
                ...
                self.sign.pending_append_only_offer = false;
                Task::none()
            }
            Message::EditAnyway => {
                self.sign.append_only = Some(crate::signing::AppendOnlyMode::EditAnyway);
                self.sign.pending_append_only_offer = false;
                Task::none()
            }
            Message::ToggleSignaturePanel => {
                self.sign.signature_panel_open = !self.sign.signature_panel_open;
                Task::none()
            }
            Message::ToggleSignatureDetail(index) => {
                self.sign.signature_panel_expanded =
                    if self.sign.signature_panel_expanded == Some(index) {
                        None
                    } else {
                        Some(index)
                    };
            ...
            Message::CopySignatureFingerprint(index) => {
                let Some(entry) = self.sign.document_signatures.get(index) else {
                    return Task::none();
                };
            ...
            Message::OverviewThumbDragged(offset) => self.handle_overview_thumb_dragged(offset),
            Message::OverviewScrolled(offset) => self.handle_overview_scrolled(offset),
            Message::ToggleOverview => self.handle_toggle_overview(),
            Message::GoToFromOverview(slide) => self.handle_goto_from_overview(slide),
            ...
            // ---- Speech (issue #20) ----
            Message::SpeakToggleScope(scope) => self.handle_speak_toggle_scope(scope),
            Message::SpeakStop => self.handle_speak_stop(),
            Message::SpeakSkip(direction) => self.handle_speak_skip(direction),
            Message::SetSpeechRate(rate) => self.handle_set_speech_rate(rate),
            Message::SetSpeechVoice(id) => self.handle_set_speech_voice(id),
            Message::SetSpeechLanguage(language) => self.handle_set_speech_language(language),
            Message::DownloadVoice(id) => self.handle_download_voice(id),
            Message::CancelVoiceDownload => self.handle_cancel_voice_download(),
            Message::ClearVoiceDownload => self.handle_clear_voice_download(),
            Message::RemoveVoice(id) => self.handle_remove_voice(id),
            Message::AnswerVoicePrompt(download) => self.handle_answer_voice_prompt(download),
            Message::ForgetDeclinedLanguages => self.handle_forget_declined_languages(),
            Message::ToggleSpeechLanguage(tag) => self.handle_toggle_speech_language(tag),
    ...
    fn rung_active(
        ...
        match rung {
            ...
            Rung::SignDialog => self.sign.flow.is_some(),
        ...
        match event {
            ...
            RenderEvent::Frame {
                ...
                if was_thumbnail {
                    ...
                    let (handle, held_bytes) = thumbnails::encoded_thumbnail(&frame);
    ...
    fn put_down_document(&mut self) {
        ...
        // (§31.3, A9).
        self.sign.append_only = None;
        self.sign.pending_append_only_offer = false;
        // §77.6: this used to set only `self.sign.flow = None`, leaving the
        ...
        // down.
        self.end_sign_flow();
        self.sign.document_signatures.clear();
        self.reader.set_signed_fields(Vec::new());
        self.sign.signature_panel_open = false;
        self.sign.signature_panel_expanded = None;
    ...
    fn finish_document_prepare(&mut self) -> Task<Message> {
        ...
        self.sign.document_signatures = prepared.signatures;
        ...
        for told in told {
            match told {
                ...
                crate::reader_link::Told::Saved {
                    ...
                    if self.sign.temp.as_deref() == Some(saved.path.as_path()) {
                        self.save_reviewed = false;
                        self.sign.source = Some(saved.path.clone());
                    ...
                    // writes to `self.sign.temp` — `signing_scratch_destination`
                    // is the only way to reach this save with `self.sign.flow`
                    ...
                    // `continue`d. `self.sign.flow` cannot still be
                ...
                crate::reader_link::Told::Failed { message, fatal } => {
                    ...
                    // Dropped here; the `notify` below is what says why, and
        ...
        match message {
            ...
            ProfileMsg::Save => {
                ...
                if let Some(id) = editor.editing_id.clone() {
                    ...
                    self.sign.unlocked_credentials.remove(&id);
                    self.signature_profile_editor = None;
                    self.persist();
                    return Task::none();
                }
...
/// The save picker, opened on a name that has already been worked out —
/// `crate::signing::signed_destination`'s, which steps aside from anything
/// already on disk rather than proposing to overwrite it.
async fn pick_pdf_named(suggested: PathBuf) -> Option<PathBuf> {
    ...
    let name = suggested
        ...
        .unwrap_or_else(|| "document-signed.pdf".to_string());
    rfd::AsyncFileDialog::new()
        .add_filter("PDF", &["pdf"])
        .set_directory(directory)
        .set_file_name(name)
