//! Platform adapter selection and native window plumbing for the display
//! extension.
//!
//! This is the one place above the platform boundary that used to need
//! `cfg(target_os = ...)`: choosing which [`pulpit_display`] backend to
//! connect to, and translating between Iced's window handles and the
//! adapters' `NativeWindow` ids. It is adapter selection, not a view or a
//! state transition, so it lives here rather than under `display.rs`, which
//! re-exports it for the rest of the crate.

use std::sync::Arc;

use iced::window;
use pulpit_display::backend::NativeWindow;
use pulpit_display::{Capabilities, DisplayBackend, NullBackend, Role};

/// Choose the platform adapter for this session.
pub fn detect_backend() -> (Arc<dyn DisplayBackend>, Capabilities) {
