...
/// Serialise a layout for export.
pub fn export_to_string(layout: &Layout) -> Result<String, StoreError> {
    serde_json::to_string_pretty(&LayoutFile::from_layout(layout))
        .map_err(|e| StoreError::Parse(e.to_string()))
}

/// Parse, migrate and validate an exported layout.
///
/// Unknown widget types and invalid structures block; ordinary validation
/// warnings are returned alongside the layout and do not block.
pub fn import_from_string(text: &str) -> Result<(Layout, Vec<Issue>), StoreError> {
    ...
    // An unknown widget type fails here, which is what we want: it is a
    // blocking condition, reported in the message.
...
mod tests {
    ...
    fn a_missing_format_version_is_migrated_forward() {
        let (_directory, mut store) = store();
        let text = export_to_string(&custom_layout("Legacy")).unwrap();
        let value: serde_json::Value = serde_json::from_str(&text).unwrap();
        let mut object = value.as_object().unwrap().clone();
        object.remove("format_version");
        let legacy = serde_json::to_string(&object).unwrap();
    ...
    #[test]
    fn an_unknown_widget_type_blocks_the_import() {
        let (_directory, mut store) = store();
        let text = export_to_string(&custom_layout("Alien")).unwrap();
        let tampered = text.replace("current-slide", "hologram-projector");
